...
public class Download {
    ...
    public static void startNoNotification(Context context, String title, String url){
        start(context, title, url, null, -1);
    }

    public static void startForce(Context context, String title, String url){
        start(context, title, url, null, -1,true);
    }
    ...
    public static void start(final Context context, String title, String url, String saveFileName, int logoResId){
        ...
        intent.putExtra(DownloadService.KEY_FORCE, false);
        if(logoResId != 0){
    ...
    public static void start(final Context context, String title, String url, String saveFileName, int logoResId, boolean forceUpdate){
        Intent intent = new Intent(context, DownloadService.class);
        intent.putExtra(DownloadService.KEY_TITLE, title);
        intent.putExtra(DownloadService.KEY_URL, url);
        intent.putExtra(DownloadService.KEY_SAVE_NAME, saveFileName);
        intent.putExtra(DownloadService.KEY_FORCE, forceUpdate);
        if(logoResId != 0){
            intent.putExtra(DownloadService.KEY_LOGO, logoResId);
        }
        context.startService(intent);
